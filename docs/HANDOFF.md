...
via `/usr/bin/security find-generic-password -w` (no access prompt, unlike
`SecItemCopyMatching` from an ad-hoc-signed build) and cached only in process memory until expiry. A 401/403 forces one Keychain
